...
entity RiscCanFdCPU is    
    ...
    port(
        ...
        can_data_write_en : out std_logic;
        can_data_address  :out std_logic_vector(DATA_ADDRESS_WIDTH-1 downto 0);
        can_data_write    : out std_logic_vector(CPU_DATA_WIDTH-1 downto 0);
        
        udp_data_write_en : out std_logic;
        udp_data_address  : out std_logic_vector(4 downto 0);
        udp_data_write    : out std_logic_vector(CPU_DATA_WIDTH-1 downto 0);
        
        gpo_out           : out std_logic_vector(CPU_DATA_WIDTH-1 downto 0);
        
        dis_out           : out std_logic_vector(CPU_DATA_WIDTH-1 downto 0);
        erc_capture_out   : out std_logic_vector(7 downto 0);
        erc_int           : out std_logic
    );        
end RiscCanFdCPU;

architecture behavioral of RiscCanFdCPU is
    ...
    type id_ex_type is record
        control_cfd_enb: std_logic;                     --added by CoPo
        control_cfd_fct: std_logic_vector(6 downto 0);  --added by CoPo
    ...
    signal id_control_cfd_enb: std_logic                   ; --added by CoPo
    signal id_control_cfd_fct: std_logic_vector(6 downto 0); --added by CoPo
    ...
    signal ex_control_cfd_enb: std_logic                   ; --added by CoPo
    signal ex_control_cfd_fct: std_logic_vector(6 downto 0); --added by CoPo
...
begin
    ...
    begin
        if rising_edge(clk) then
            if reset_n = '0' then
                ...
                id_ex_reg <= ('0',"0000000","00", '0', '0', '0', '0', '0', others => (others => '0')); -- 0 and 0000000 added by CoPo
    ...
    ctrl_unit: entity work.control_unit
        port map(
            opcode => if_id_reg.instruction.opcode,
            funct7 => if_id_reg.instruction.funct7,
            id_control_alu_op  => id_control_alu_op,
            id_control_alu_src => id_control_alu_src,
            id_control_mem_read => id_control_mem_read,
            id_control_mem_write=> id_control_mem_write,
            id_control_reg_write=> id_control_reg_write,
            id_control_mem_to_reg => id_control_mem_to_reg,
            id_control_is_branch => id_control_is_branch,
            id_control_cfd_enb => id_control_cfd_enb,
            id_control_cfd_fct => id_control_cfd_fct
        ); 
    alu_ctrl: entity work.alu_control
        port map(
            id_ex_reg_alu_control => id_ex_reg.alu_control,
            id_ex_reg_control_alu_op => id_ex_reg.control_alu_op,
            ex_alu_control => ex_alu_control
        );
    ---------------entity added by CoPo----------------------------------------------
    
    cfd_unit: entity work.canfd_unit
        port map(
            --reset => reset_n_neg,
            reset => reset_n,
            clock => clk,
            can_rx => can_rx,
            can_tx => can_tx,
            cfd_enb => ex_control_cfd_enb,
            cfd_fct => ex_control_cfd_fct,
            cfd_op1 => ex_alu_left_operand,
            cfd_op2 => ex_alu_right_operand,
            datamem_write_en => can_data_write_en,
            datamem_write_data => can_data_write,
            datamem_write_address =>  can_data_address,
            udpmem_write_en => udp_data_write_en,
            udpmem_write_data => udp_data_write,
            udpmem_write_address => udp_data_address,
            erc_capture_out => erc_capture_out,
            erc_int => erc_int
        );
    
    --------------process added by CoPo----------------------------------------------
    
    cfd_ctrl: entity work.cfd_control
        port map(
            id_ex_reg_control_cfd_enb => id_ex_reg.control_cfd_enb,
            id_ex_reg_control_cfd_fct => id_ex_reg.control_cfd_fct,
            ex_control_cfd_enb => ex_control_cfd_enb,
            ex_control_cfd_fct => ex_control_cfd_fct
        );
    ---------------------------------------------------------------------------------
    ...
    id_ex_next <= (
        control_cfd_enb => id_control_cfd_enb, --added by CoPo
        control_cfd_fct => id_control_cfd_fct, --added by CoPo
